...
export function initPlayerApp(root: HTMLElement) {
  ...
  const renderLogin = () => {
      ...
      <div class="player-container flex-center" style="height: 100vh; flex-direction: column;">
        ...
        <div style="background: rgba(0,0,0,0.5); padding: 3rem; border-radius: 12px; display: flex; flex-direction: column; gap: 1.5rem; width: 400px; max-width: 90%;">
          <div style="display: flex; flex-direction: column; gap: 0.5rem;">
            <label style="color: var(--color-text-secondary); font-family: var(--font-heading);">Team Name</label>
            <input type="text" id="login-team-id" placeholder="e.g. My Awesome Team" style="padding: 1rem; border-radius: 6px; border: 1px solid rgba(255,255,255,0.2); background: rgba(0,0,0,0.3); color: white; font-family: var(--font-body); font-size: 1.2rem;">
    ...
    document.getElementById('btn-join-session')?.addEventListener('click', () => {
      ...
      const btn = document.getElementById('btn-join-session') as HTMLButtonElement;
      btn.disabled = true;
      btn.innerText = "Joining...";

      // Register the team first
      fetch(`${API_BASE}/api/team/register`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ team_name: tId })
      })
      .then(res => {
      ...
      .then(teamData => {
        state.sessionId = sId;
        state.teamId = teamData.team.team_id; // Use the generated team_id
        state.status = 'loading';
        
        // Now verify session exists
        return fetch(`${API_BASE}/api/session/${sId}/state`);
      })
      .then(res => {
         if(res.ok) {
           startPolling();
